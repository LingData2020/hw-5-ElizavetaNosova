...
```{r 1.1}
ggplot(shva, aes(f1, f2, colour = vowel)) + geom_point()
```
...
```{r 1.2}
# f1 boxplot
ggplot(shva, aes(vowel, f1, colour = vowel)) + geom_boxplot(outlier.color = NULL) + coord_flip()
```

```{r 1.2}
# f2 boxplot
ggplot(shva, aes(vowel, f2, colour = vowel)) + geom_boxplot(outlier.color = NULL) + coord_flip()
```

### 1.3 
Calculate Pearson's correlation of `f1` and `f2` (all data)
```{r 1.3}
chisq.test(shva$f1, shva$f2)
```
...
shva %>%
  group_by(vowel) %>%
  summarise(chisq.test(f1,f2)$statistic)

shva %>%
  group_by(vowel) %>%
  summarise(chisq.test(f1,f2)$p.value)
```
...
```{r 2.1.2}
shva$predicted_f2 = 1639.70215 -0.42875*shva$f1
```

### 2.1.3 
Provide the adjusted R$^2$
```{r 2.1.3}
mean((shva$f2-shva$predicted_f2)^2)
...
```{r 2.1.4}
ggplot(shva, aes(f1, f2)) + geom_point() + geom_smooth(method = "lm")
```
...
shva$dummy_a = 0
shva$dummy_y = 0
shva$dummy_a[shva$vowel=='a'] =1
shva$dummy_y[shva$vowel=='y'] = 1
fit1 <- lm(data = shva, f2 ~ f1 + dummy_a + dummy_y)
summary(fit1)
```

### 2.2.2 
What is the intercept of the model?
Гласный 'А' c первой формантой, равной нулю

### 2.2.3 
Provide the adjusted R$^2$
```{r 2.2.3}
shva$predicted_f2 = 1.249e+03 + 9.600e-02*shva$f1 + 1.174e+02*shva$dummy_a + 1.595e+02*shva$dummy_y
mean((shva$f2-shva$predicted_f2)^2)
```

### 2.2.4 
Write down your general conclusions about the relationship between `f1`, `f2`, and `vowels`.
Судя по графикам, первой форманты и типа гласных недостаточно, чтобы с хорошей точностью предсказывать вторую форманту. Предсказание с использованием двух признаков работает лучше, чем предсказание только по первой форманте. 
Также по данным можно сделать тривиальный вывод о том, что первая форманта всегда ниже второй. Интервалы, в которых ожидаются значения формант, зависят от конкретного гласного
...
Read more on the constructions in [Levhina, Geerarts, Speelman 2014](https://www.academia.edu/7287585/Dutch_causative_constructions_Qua
### 3.0 
...
```{r 3.1}
nrow(filter(d_caus, Aux=='doen', CeSynt=='Clause'))
```

Чтобы применить Chi-squared Test, в каждой клетке таблицы должно быть не меньше 5 примеров. В клетке "doen, Clause" 0 примеров, поэтому нужно использовать другой тип теста. R выберет другой тест автоматически.
```{r 3.1}
fisher.test(table(d_caus$Aux, d_caus$CeSynt))
```
...
```
p-value низкое, отвергаем нулевую гипотезу о независимости переменных
### 3.3
...
```{r 3.4.1}
res = fisher.test(d_caus$Aux, d_caus$EPTrans)
res$estimate
```

What can you say about odds ratio of expected frequencies for arbitrary data?
Кажется, что doen при переходных глаголах встречается реже, чем ожидалось бы, если б нулевая гипотеза о независимости переменных была верна.
...
```
We have not found a significant association between variables Aux and EPTrans (p > 0.001).  The odds of ... were ... times higher / lower in (group ....) than in (group ....). Effect size is large / medium / small (Cramer's V = ....).
```
...
```{r 3.7}
library(vcd)
mosaic(d_caus$Aux ~ d_caus$EPTrans)
```
...
```
Вероятность того, что мы совершим ошибку первого рода, применяя тест несколько раз, существенно выше, чем выбранный нами допустимый порог p-value.
Кроме того, формула, которая используется в тесте, зависит от количества степней свободы (от формы таблицы). При попарных сравнениях количество степеней свободы отличается от количества степеней свободы в полной таблице. Будет непонято, как соотнести результаты, полученные при попарном сравнении и на всех данных.
```

### 3.9 
Provide a short text (300 words) describing the hypothesis of this study and the results of your analysis.
```{r 3.9}
Нулевыми гипотезами исследования следует считать предположение, что в немецком языке употребление глаголов doen и laten не зависит от характеристик других компонентов предложения (одушевленности/неодушевленности актантов, типа предиката), типа синтаксической конструкции, регионального варианта немецкого и тематики текстов.
Альтернативные гипотезы - зависимость выбора слова от вышеупомянутых факторов.
В домашнем задании я воспроизвела часть исследования. Удалось показать, что выбор слова коррелирует с переходностью и синтаксической ролью. 
```